...
public class Updater
{
    ...
    private static Uri GetBaseAddress() => new("https://adventofcode.com");
    ...
    private static string GetSession()
    {
        if (!Environment.GetEnvironmentVariables()
                .Contains("SESSION"))
        {
            throw new AocCommunicationError("Specify SESSION environment variable");
        }
    ...
    public async Task Upload(ISolver solver)
    {
        ...
        else
        {
            ...
            var article = document.Body.QuerySelector("body > main > article")
                .TextContent;
    ...
    private static async Task<Problem> DownloadProblem(IBrowsingContext context, Uri baseUri, int year, int day)
    {
        ...
        var input = await context.GetService<IDocumentLoader>()
            .FetchAsync(
                new DocumentRequest(new Url(baseUri + $"{year}/day/{day}/input")))
            .Task;

        if (input.StatusCode != HttpStatusCode.OK)
        {
            throw new AocCommunicationError(
